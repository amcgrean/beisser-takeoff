...
// Body: { picker_id: number, barcode: string }
// No auth required — kiosk runs on trusted in-store devices
export async function POST(req: NextRequest) {
  const body = await req.json() as { picker_id?: number; barcode?: string };
  ...
  // 1. Check for existing incomplete pick for this barcode (branch-scoped)
  ...
  // 2. Look up sale_type and handling_code from ERP
  let saleType: string | null = null;
  let handlingCode: string | null = null;
  try {
    const soRows = await sql<{ sale_type: string | null; handling_code: string | null }[]>`
  ...
  } catch { /* ERP lookup failure → fall back to Yard */ }

  const isWillCall = saleType?.toUpperCase() === 'WILLCALL';
  const pickTypeId = isWillCall ? WILL_CALL_TYPE_ID : pickTypeFromHandlingCode(handlingCode);
